...
export function getIngestionMaskingConfig(
  ...
  const callbackUrl = e.HANZO_INGESTION_MASKING_CALLBACK_URL;
  if (!callbackUrl) {
    return null;
  }
  ...
  return {
    callbackUrl,
    timeoutMs: e.HANZO_INGESTION_MASKING_CALLBACK_TIMEOUT_MS,
    failClosed: e.HANZO_INGESTION_MASKING_CALLBACK_FAIL_CLOSED === "true",
    maxRetries: e.HANZO_INGESTION_MASKING_MAX_RETRIES,
    propagatedHeaders: e.HANZO_INGESTION_MASKING_PROPAGATED_HEADERS,
  };
}
...
async function makeCallbackRequest<T>(params: {
  ...
  try {
    ...
    recordHistogram(
      "hanzo.ingestion.masking.callback_duration_ms",
      duration,
      {
...
export async function applyIngestionMasking<T>(
  ...
  for (let attempt = 1; attempt <= config.maxRetries + 1; attempt++) {
    ...
    if (result.success) {
      recordIncrement("hanzo.ingestion.masking.success", 1, {
        attempt: attempt.toString(),
      });
  ...
  recordIncrement("hanzo.ingestion.masking.failure", 1, {
