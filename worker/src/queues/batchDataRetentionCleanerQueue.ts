...
export const batchDataRetentionCleanerProcessor: Processor = async (job) => {
  if (job.name === QueueJobs.BatchDataRetentionCleanerJob) {
    ...
    if (BATCH_DATA_RETENTION_TABLES.indexOf(table) === -1) {
      logger.error(`Invalid table name for BatchDataRetentionCleaner: ${table}`);
      return;
    }

    logger.info(`Executing BatchDataRetentionCleaner for ${table}`);
    try {
      await BatchDataRetentionCleaner.processBatch(table as BatchDataRetentionTable);
    } catch (error) {
      logger.error(`Error executing BatchDataRetentionCleaner for ${table}`, error);
      throw error;
    }
  }
};
