...
const processPostHogTraces = async (config: PostHogExecutionConfig) => {
  ...
  logger.info(`[INSIGHTS] Sending traces for project ${config.projectId} to PostHog`);
  ...
  posthog.on("error", (error) => {
    logger.error(`[INSIGHTS] Error sending traces to PostHog for project ${config.projectId}: ${error}`);
  ...
  for await (const trace of traces) {
    ...
    if (count % 10000 === 0) {
      await posthog.flush();
      if (sendError) throw sendError;
      logger.info(`[INSIGHTS] Sent ${count} traces to PostHog for project ${config.projectId}`);
    }
  }
  await posthog.flush();
  if (sendError) throw sendError;
...
const processPostHogGenerations = async (config: PostHogExecutionConfig) => {
  ...
  logger.info(`[INSIGHTS] Sending generations for project ${config.projectId} to PostHog`);
  ...
  let sendError: Error | undefined;
  posthog.on("error", (error) => {
    logger.error(`[INSIGHTS] Error sending generations to PostHog for project ${config.projectId}: ${error}`);
    sendError = error instanceof Error ? error : new Error(String(error));
  });
  ...
  for await (const generation of generations) {
    ...
    if (count % 10000 === 0) {
      await posthog.flush();
      if (sendError) throw sendError;
      logger.info(`[INSIGHTS] Sent ${count} generations to PostHog for project ${config.projectId}`);
    }
  }
  await posthog.flush();
  if (sendError) throw sendError;
...
const processPostHogScores = async (config: PostHogExecutionConfig) => {
  ...
  logger.info(`[INSIGHTS] Sending scores for project ${config.projectId} to PostHog`);
  ...
  let sendError: Error | undefined;
  posthog.on("error", (error) => {
    logger.error(`[INSIGHTS] Error sending scores to PostHog for project ${config.projectId}: ${error}`);
    sendError = error instanceof Error ? error : new Error(String(error));
  });
  ...
  for await (const score of scores) {
    ...
    if (count % 10000 === 0) {
      await posthog.flush();
      if (sendError) throw sendError;
      logger.info(`[INSIGHTS] Sent ${count} scores to PostHog for project ${config.projectId}`);
    }
  }
  await posthog.flush();
  if (sendError) throw sendError;
...
const processPostHogEvents = async (config: PostHogExecutionConfig) => {
  ...
  logger.info(`[INSIGHTS] Sending events for project ${config.projectId} to PostHog`);
  ...
  let sendError: Error | undefined;
  posthog.on("error", (error) => {
    logger.error(`[INSIGHTS] Error sending events to PostHog for project ${config.projectId}: ${error}`);
    sendError = error instanceof Error ? error : new Error(String(error));
  });
  ...
  for await (const analyticsEvent of events) {
    ...
    if (count % 10000 === 0) {
      await posthog.flush();
      logger.info(`[INSIGHTS] Sent ${count} events to PostHog for project ${config.projectId}`);
    }
  }
  await posthog.flush();
  if (sendError) throw sendError;
...
export const handleInsightsIntegrationProjectJob = async (
  ...
  logger.info(`[INSIGHTS] Processing PostHog integration for project ${projectId}`);
  ...
  if (!postHogIntegration) {
    logger.warn(`[INSIGHTS] Enabled PostHog integration not found for project ${projectId}`);
    return;
  }

  if (!postHogIntegration.project) {
    logger.warn(`[INSIGHTS] Project not found for PostHog integration ${projectId}`);
    return;
  }
  ...
  try {
    ...
    logger.error(
      `[INSIGHTS] PostHog integration for project ${projectId} has invalid hostname: ${postHogIntegration.posthogHostName}. Error: ${error instanceof Error ? error.message : String(error)}`,
    );
    throw new Error(
  ...
  try {
    ...
    logger.info(`[INSIGHTS] PostHog integration processing complete for project ${projectId}`);
  } catch (error) {
    logger.error(`[INSIGHTS] Error processing PostHog integration for project ${projectId}`, error);
    throw error;
  }
};
