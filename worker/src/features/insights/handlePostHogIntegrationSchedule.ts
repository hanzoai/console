...
export const handleInsightsIntegrationSchedule = async () => {
  ...
  if (postHogIntegrationProjects.length === 0) {
    logger.info("[INSIGHTS] No PostHog integrations ready for sync");
    return;
  }
  ...
  logger.info(`[INSIGHTS] Scheduling ${postHogIntegrationProjects.length} PostHog integrations for sync`);
