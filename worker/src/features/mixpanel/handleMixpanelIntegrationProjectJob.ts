...
import { transformTraceForMixpanel, transformGenerationForMixpanel, transformScoreForMixpanel } from "./transformers";
...
const processMixpanelTraces = async (config: MixpanelExecutionConfig) => {
  ...
  for await (const trace of traces) {
    ...
    if (count % 1000 === 0) {
      ...
      logger.info(`Sent ${count} traces to Mixpanel for project ${config.projectId}`);
    }
  }
  await mixpanel.flush();
  logger.info(`Sent ${count} traces to Mixpanel for project ${config.projectId}`);
...
const processMixpanelGenerations = async (config: MixpanelExecutionConfig) => {
  ...
  logger.info(`Sending generations for project ${config.projectId} to Mixpanel`);
  ...
  for await (const generation of generations) {
    ...
    if (count % 1000 === 0) {
      await mixpanel.flush();
      logger.info(`Sent ${count} generations to Mixpanel for project ${config.projectId}`);
    }
  }
  await mixpanel.flush();
  logger.info(`Sent ${count} generations to Mixpanel for project ${config.projectId}`);
...
const processMixpanelScores = async (config: MixpanelExecutionConfig) => {
  ...
  for await (const score of scores) {
    ...
    if (count % 1000 === 0) {
      await mixpanel.flush();
      logger.info(`Sent ${count} scores to Mixpanel for project ${config.projectId}`);
    }
  }
  await mixpanel.flush();
  logger.info(`Sent ${count} scores to Mixpanel for project ${config.projectId}`);
...
export const handleMixpanelIntegrationProjectJob = async (
  ...
  if (!mixpanelIntegration) {
    logger.warn(`Enabled Mixpanel integration not found for project ${projectId}`);
    return;
  }
  ...
  logger.info(`Mixpanel integration processing complete for project ${projectId}`);
};
