...
import type { SharedEnv } from "@hanzo/shared/src/env";
import {
...
const VALID_EE_LICENSE_KEY = "hanzo_ee_test-license-key";

// Default test env with masking disabled
const defaultTestEnv: SharedEnv = {
  HANZO_INGESTION_MASKING_CALLBACK_URL: undefined,
  HANZO_INGESTION_MASKING_CALLBACK_TIMEOUT_MS: 500,
  HANZO_INGESTION_MASKING_CALLBACK_FAIL_CLOSED: "false",
  HANZO_INGESTION_MASKING_MAX_RETRIES: 1,
  HANZO_INGESTION_MASKING_PROPAGATED_HEADERS: [],
  NEXT_PUBLIC_HANZO_CLOUD_REGION: undefined,
  HANZO_EE_LICENSE_KEY: undefined,
} as SharedEnv;
...
describe("Ingestion Masking", () => {
  ...
  describe("isIngestionMaskingEnabled", () => {
    it("should return false when callback URL is not configured", () => {
      const testEnv = createTestEnv({
        HANZO_EE_LICENSE_KEY: VALID_EE_LICENSE_KEY,
      });
    ...
    it("should return false when EE license is not available", () => {
      const testEnv = createTestEnv({
        HANZO_INGESTION_MASKING_CALLBACK_URL:
  ...
  describe("applyIngestionMasking", () => {
    ...
    it("should include X-Hanzo-Org-Id and X-Hanzo-Project-Id headers", async () => {
      const testEnv = createTestEnv({
        HANZO_INGESTION_MASKING_CALLBACK_URL:
          "https://masking.example.com/success",
        HANZO_EE_LICENSE_KEY: VALID_EE_LICENSE_KEY,
      ...
      expect(request?.headers["x-hanzo-org-id"]).toBe("test-org-456");
      expect(request?.headers["x-hanzo-project-id"]).toBe(
        "test-project-123",
      );
    });
    ...
    it("should return original data on HTTP 500 with fail-open (default)", async () => {
      const testEnv = createTestEnv({
        ...
        HANZO_EE_LICENSE_KEY: VALID_EE_LICENSE_KEY,
        HANZO_INGESTION_MASKING_CALLBACK_FAIL_CLOSED: "false",
        HANZO_INGESTION_MASKING_MAX_RETRIES: 0,
      });
    ...
    it("should return failure on HTTP 500 with fail-closed", async () => {
      const testEnv = createTestEnv({
        HANZO_INGESTION_MASKING_CALLBACK_URL:
          "https://masking.example.com/error",
        HANZO_EE_LICENSE_KEY: VALID_EE_LICENSE_KEY,
        HANZO_INGESTION_MASKING_CALLBACK_FAIL_CLOSED: "true",
        HANZO_INGESTION_MASKING_MAX_RETRIES: 0,
      });
    ...
    it("should retry on failure", async () => {
      const testEnv = createTestEnv({
        ...
        HANZO_EE_LICENSE_KEY: VALID_EE_LICENSE_KEY,
        HANZO_INGESTION_MASKING_CALLBACK_FAIL_CLOSED: "false",
        HANZO_INGESTION_MASKING_MAX_RETRIES: 2,
      });
    ...
    it("should handle timeout with fail-open", async () => {
      const testEnv = createTestEnv({
        ...
        HANZO_EE_LICENSE_KEY: VALID_EE_LICENSE_KEY,
        HANZO_INGESTION_MASKING_CALLBACK_TIMEOUT_MS: 100, // Short timeout
        HANZO_INGESTION_MASKING_CALLBACK_FAIL_CLOSED: "false",
        HANZO_INGESTION_MASKING_MAX_RETRIES: 0,
      });
