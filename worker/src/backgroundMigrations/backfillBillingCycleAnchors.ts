...
export default class BackfillBillingCycleAnchors implements IBackgroundMigration {
  ...
  async validate(_args: Record<string, unknown>): Promise<{ valid: boolean; invalidReason: string | undefined }> {
    ...
    if (!env.STRIPE_SECRET_KEY) {
      return {
        valid: false,
        invalidReason: "Migration requires Stripe integration in cloud environment (STRIPE_SECRET_KEY not set)",
      };
    }
    ...
    try {
      ...
      logger.error("[Background Migration] Failed to check for required columns", { error });
      return {
        valid: false,
  ...
  async run(args: Record<string, unknown>): Promise<void> {
    ...
    try {
      ...
      for (const org of orgsToBackfill) {
        ...
        const activeSubscriptionId = parsedOrg.cloudConfig?.stripe?.activeSubscriptionId;
      ...
      if (orgsWithSubscription.length > 0) {
        if (!stripeClient) {
          logger.error("[Background Migration] Stripe client not available, skipping subscription-based backfill");
          errors += orgsWithSubscription.length;
        } else {
          const result = await this.backfillFromStripe(stripeClient, orgsWithSubscription);
  ...
  private async backfillFromStripe(
    ...
    for (const batch of batches) {
      ...
      const results = await Promise.allSettled(batch.map((org) => this.backfillSingleOrgFromStripe(stripe, org)));
  ...
  private async backfillSingleOrgFromStripe(
    ...
    try {
      ...
      if (!subscription.billing_cycle_anchor) {
        logger.warn(`[Background Migration] No billing_cycle_anchor found for subscription ${org.subscriptionId}`, {
          orgId: org.id,
      ...
      logger.debug(`[Background Migration] Backfilled ${org.id} with billing_cycle_anchor from Stripe: ${anchorDate}`);
      return { success: true };
    } catch (error: any) {
      ...
      if (attempt < maxAttempts && (status === 429 || code === "rate_limit")) {
        ...
        return this.backfillSingleOrgFromStripe(stripe, org, attempt + 1, maxAttempts);
      }

      // Log and fail for other errors
      logger.error(`[Background Migration] Failed to backfill ${org.id} from Stripe`, {
        error,
        orgId: org.id,
        subscriptionId: org.subscriptionId,
        attempt,
