...
import { type EvalTemplate, variableMapping, observationVariableMapping } from "@hanzo/console-core";
...
import { useUserFacingTarget, useEvaluatorTargetState } from "@/src/features/evals/hooks/useEvaluatorTarget";
import {
...
const addPropagationWarnings = (
  ...
  return columns.map((col) => {
    if (!allowPropagationFilters && COLUMN_IDENTIFIERS_THAT_REQUIRE_PROPAGATION.has(col.id)) {
      return {
        ...col,
        alert: {
          ...
          content: (
            <>
              This filter requires JS SDK &ge; 4.0.0 or Python SDK &ge; 3.0.0 with attribute propagation enabled. Please{" "}
              <a
                href="https://hanzo.ai/docs/integrations/native/opentelemetry#propagating-attributes"
                target="_blank"
...
const ObservationsTable = lazy(() => import("@/src/components/table/use-cases/observations"));

const EventsTable = lazy(() => import("@/src/features/events/components/EventsTable"));
...
const ObservationsPreview = memo(
  ({ projectId, filterState }: { projectId: string; filterState: z.infer<typeof singleFilter>[] }) => {
    ...
    return (
      <>
        <div className="flex flex-col items-start gap-1">
          <FormDescription>Sample over the last 24 hours that match filters</FormDescription>
        </div>
        <div className="mb-4 flex max-h-[30dvh] w-full flex-col overflow-hidden border-b border-l border-r">
          <Suspense fallback={<Skeleton className="h-[30dvh] w-full" />}>
            {isv4Enabled ? (
...
export const InnerEvaluatorForm = (props: {
  ...
  renderFooter?: (params: { isLoading: boolean; formError: string | null }) => React.ReactNode;
  oldConfigId?: string;
}) => {
  ...
  const { userFacingTarget, setUserFacingTarget, useOtelDataForExperiment, setUseOtelDataForExperiment } =
    useUserFacingTarget(props.existingEvaluator?.targetObject);

  const { traceFilterOptions, observationEvalFilterOptions, experimentEvalFilterOptions, datasetFilterOptions } =
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      filter: props.existingEvaluator?.filter
        ...
        : (props.existingEvaluator?.targetObject ?? EvalTargetObject.EVENT) === EvalTargetObject.TRACE
          ? // For new trace evaluators, exclude internal environments by default
            DEFAULT_TRACE_FILTER
          : (props.existingEvaluator?.targetObject ?? EvalTargetObject.EVENT) === EvalTargetObject.EVENT
      ...
      mapping: props.existingEvaluator?.variableMapping
        ? isEventTarget(props.existingEvaluator.targetObject) ||
          ...
          ? z.array(observationVariableMapping).parse(props.existingEvaluator.variableMapping)
  ...
  useEffect(() => {
    if (props.evalTemplate && form.getValues("mapping").length === 0) {
      ...
      form.setValue(
        ...
        props.evalTemplate.vars.map((v) => ({
          templateVariable: v,
          consoleObject: isLegacyEvalTarget(target) ? ("trace" as const) : undefined,
  ...
  >(() => targetState.getAvailableVariables(props.existingEvaluator?.targetObject ?? EvalTargetObject.EVENT));
  ...
  function onSubmit(values: z.infer<typeof evalConfigFormSchema>) {
    ...
    const validatedVarMapping = validateAndTransformVariableMapping(values.mapping, values.target as EvalTargetObject);
    ...
    const status = isModern ? (values.runOnLive ? JobConfigState.ACTIVE : JobConfigState.INACTIVE) : undefined;
  ...
  function handleAndResolveTarget(value: string) {
    const newUserFacingTarget = value as "trace" | "event" | "offline-experiment";
    ...
    if (newUserFacingTarget === "trace" && !props.evalCapabilities.hasLegacyEvals && props.mode !== "edit") {
    ...
    if (newUserFacingTarget === "trace") {
      ...
      actualTarget = useOtelDataForExperiment ? EvalTargetObject.EXPERIMENT : EvalTargetObject.DATASET;
    }

    // Transform variable mapping for new target type
    const currentMapping = form.getValues("mapping");
    const newMapping = targetState.transformMapping(currentMapping, actualTarget);
      ...
      {!props.hideTargetSection && (
        <Card className="flex max-w-full flex-col gap-2 overflow-y-auto p-4">
          <div className="flex flex-col gap-4">
                      ...
                      >
                        <TabsList className="grid w-fit max-w-fit grid-flow-col gap-4">
                          ...
                          >
                            ...
                            Observations
                            <Badge variant="secondary" size="sm" className="border border-border font-normal">
                              Beta
                            </Badge>
                          </TabsTrigger>
                          {allowLegacy && (
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <Tabs
                    ...
                    onValueChange={(value) => {
                      ...
                      const actualTarget = useOtel ? EvalTargetObject.EXPERIMENT : EvalTargetObject.DATASET;
            ...
            {!props.hideTargetSelection && props.mode !== "edit" && !props.disabled && (
              <EvalVersionCallout targetObject={form.watch("target")} evalCapabilities={props.evalCapabilities} />
            )}

            {!props.hideAdvancedSettings && isLegacyEvalTarget(form.watch("target")) && (
              <FormField
                control={form.control}
                name="timeScope"
                render={({ field }) => (
                        ...
                        <div className="items-top flex space-x-2">
                          ...
                          <div className="flex items-center gap-1.5 leading-none">
                            <label
                              ...
                              className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
                            >
                              Existing {getTargetDisplayName(form.watch("target"))}
                            </label>
                                ...
                                <Tooltip>
                                  ...
                                  <TooltipContent className="max-w-[300px] p-2">
                                    <span className="leading-4">
                                      ...
                                      {getTargetDisplayName(form.watch("target"))} once. Set up a new evaluator to
                                      re-run on existing {getTargetDisplayName(form.watch("target"))}.
                                    </span>
                                  </TooltipContent>
                                </Tooltip>
                              ) : (
                    ...
                    <div className="flex max-w-4xl flex-col gap-2">
                      <FormItem className="flex items-center justify-between rounded-lg border p-3">
                        <div className="space-y-0.5">
                          <FormLabel>
                            {isEventTarget(target) ? "Run on live incoming observations" : "Run on new experiments"}
                          </FormLabel>
                          <FormDescription>
                            Automatically evaluate new incoming {getTargetDisplayName(target)}.
                          </FormDescription>
                        </div>
                        <FormControl>
            ...
            {(isLegacyEvalTarget(form.watch("target")) || form.watch("runOnLive")) && (
              <>
                <FormField
                  control={form.control}
                  name="filter"
                  render={({ field }) => {
                    ...
                    const getFilterColumns = () => {
                      if (isEventTarget(target)) {
                        ...
                        const baseColumns = observationEvalFilterColsWithOptions(observationEvalFilterOptions);
                        return addPropagationWarnings(baseColumns, allowPropagationFilters);
                        ...
                        return experimentEvalFilterColsWithOptions(experimentEvalFilterOptions);
                      } else {
                        // dataset (legacy non-OTEL experiments)
                        return datasetFormFilterColsWithOptions(datasetFilterOptions);
                      }
                    };
                        ...
                        <FormControl>
                          <div className="max-w-[500px]">
                            {props.disabled && !hasFilters ? (
                              ...
                              <InlineFilterBuilder
                                columnIdentifier={isDatasetTarget(target) || isTraceTarget(target) ? "name" : "id"}
                                ...
                                columnsWithCustomSelect={
                                  isEventTarget(target) || isTraceTarget(target) ? ["tags", "name"] : undefined
                                }
                              />
                            )}
                          </div>
                        ...
                        {!props.disabled && !hasFilters && (
                          <div className="align-center flex max-w-[500px] gap-1">
                            ...
                            <AlertDescription className="text-dark-yellow">
                              No filters set. This evaluator will run on all {getTargetDisplayName(target)}.
                            </AlertDescription>
                          </div>
                        )}
                        <FormMessage />
                ...
                {!props.disabled && !props.hidePreviewTable && (
                  <>
                    ...
                    {isEventTarget(form.watch("target")) && (
                      <ObservationsPreview projectId={props.projectId} filterState={form.watch("filter") ?? []} />
                    )}
                  </>
                )}
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormDescription>
                              Time between first Trace/Dataset run event and evaluation execution to ensure all data is
                              available
                            </FormDescription>
                            <FormMessage />
                          </FormItem>
  ...
  const mutationIsLoading = createJobMutation.isPending || updateJobMutation.isPending;
  ...
  const formFooter = props.renderFooter ? (
    ...
    <div className="flex w-full flex-col items-end gap-4">
      {!props.disabled ? (
        <Button type="submit" loading={mutationIsLoading} className="mt-3 max-w-fit">
  ...
  return (
    <>
      ...
      <Dialog open={showTraceConfirmDialog} onOpenChange={setShowTraceConfirmDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>You selected a legacy evaluator</DialogTitle>
          </DialogHeader>
          <DialogBody className="text-sm">
            We strongly recommend using observation evaluators. Trace evaluators will be deprecated in the future. Only
            proceed if you are sure you cannot upgrade your SDK version now.
          </DialogBody>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowTraceConfirmDialog(false)}>
              Cancel
            </Button>
            <Button
              onClick={() => {
                ...
                const consoleObject: ConsoleObject = "trace";
