...
export const llmSchemaRouter = createTRPCRouter({
  create: protectedProjectProcedure.input(CreateLlmSchemaInput).mutation(async ({ input, ctx }) => {
    try {
      ...
      if (existingSchema) {
        throw new TRPCError({
          code: "CONFLICT",
          message: "An LLM Schema with this name already exists in this project",
        });
      }
  ...
  update: protectedProjectProcedure.input(UpdateLlmSchemaInput).mutation(async ({ input, ctx }) => {
    try {
      throwIfNoProjectAccess({
        session: ctx.session,
        projectId: input.projectId,
      ...
      if (duplicateNameCheck) {
        throw new TRPCError({
          code: "CONFLICT",
          message: "Another LLM Schema with this name already exists in this project",
        });
      }
  ...
  delete: protectedProjectProcedure.input(DeleteLlmSchemaInput).mutation(async ({ input, ctx }) => {
    try {
      throwIfNoProjectAccess({
        session: ctx.session,
        projectId: input.projectId,
