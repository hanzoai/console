              ...
              <div className="flex items-start gap-2">
                <div className="flex-1 min-w-0">
                  <div className="text-body">
                    {isExpanded ? (
                      <span className="font-medium text-text-primary">Expanded</span>
                    ) : (
                      ...
                      <div className="prose prose-sm max-w-none prose-invert prose-headings:text-text-primary prose-p:text-text-secondary prose-strong:text-text-primary prose-code:text-text-primary prose-code:bg-bg-secondary prose-code:px-1 prose-code:py-0.5 prose-code:rounded prose-pre:bg-bg-secondary prose-pre:border prose-pre:border-border-secondary">
                        <ReactMarkdown
                          ...
                          components={{
                            ...
                            ul: ({children}) => <ul className="list-disc list-inside mb-2 text-body space-y-1">{children}</ul>,
                            ol: ({children}) => <ol className="list-decimal list-inside mb-2 text-body space-y-1">{children}</ol>,
                            li: ({children}) => <li className="leading-relaxed">{children}</li>,
                            code: ({children, className}) => {
                              const isInline = !className;
                              return isInline ? (
                                <code className="bg-bg-secondary px-1 py-0.5 rounded text-xs font-mono text-text-primary border border-border-secondary">
                                  {children}
                                </code>
                              ) : (
                            ...
                            pre: ({children}) => (
                              <pre className="bg-bg-secondary p-3 rounded text-xs overflow-auto border border-border-secondary mb-2 font-mono text-text-primary">
                                {children}
                              </pre>
                            ),
                            blockquote: ({children}) => (
                              <blockquote className="border-l-4 border-accent-primary pl-3 italic text-text-tertiary mb-2 bg-bg-secondary/30 py-1 rounded-r">
                                {children}
                              </blockquote>
                            ),
                            a: ({href, children}) => (
                            ...
                            table: ({children}) => (
                              <div className="overflow-auto mb-2 border border-border-secondary rounded">
                                <table className="min-w-full text-xs">{children}</table>
                              </div>
                            ),
                            th: ({children}) => (
                              <th className="border-b border-border-secondary px-2 py-1 text-left text-xs font-medium text-text-primary bg-bg-secondary">
                                {children}
                              </th>
                            ),
                            td: ({children}) => (
                              <td className="border-b border-border-secondary px-2 py-1 text-body-small">
                                {children}
                              </td>
                            ),
                          }}
