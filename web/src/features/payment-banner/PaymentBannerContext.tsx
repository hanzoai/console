...
const PaymentBannerContext = createContext<PaymentBannerContextValue | null>(null);
...
export function PaymentBannerProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <PaymentBannerContext.Provider value={{ bannerHeight, setBannerHeight }}>{children}</PaymentBannerContext.Provider>
  );
}
...
export function usePaymentBannerHeight() {
  ...
  if (!context) {
    throw new Error("usePaymentBannerHeight must be used within PaymentBannerProvider");
  }
  return context;
}
