...
const mockedUseQuery = api.scoreAnalytics.getScoreComparisonAnalytics.useQuery as jest.Mock;

function ScoreAnalyticsProbe({ includeScore2 = false }: { includeScore2?: boolean }) {
  const score = {
    name: "Correctness",
  ...
  return <div data-testid="distribution">{JSON.stringify(result.data?.distribution ?? null)}</div>;
}
...
describe("useScoreAnalyticsQuery", () => {
  ...
  it("maps true-only boolean data to the True bucket", () => {
    ...
    const distribution = JSON.parse(screen.getByTestId("distribution").textContent ?? "null");

    expect(JSON.stringify(distribution.score1)).toBe('[{"binIndex":0,"count":0},{"binIndex":1,"count":1}]');
  });
