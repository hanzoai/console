...
export function useScoreAnalyticsQuery(
  ...
  const transformedData = useMemo<ScoreAnalyticsData | null>(() => {
    ...
    const normalizeBooleanCategory = (value: string): "False" | "True" | null => {
    ...
    const buildBooleanDistribution = (
      ...
      const countByCategory = new Map<string, number>(distributionCategories.map((category) => [category, 0]));
      ...
      timeSeries.forEach((item) => {
        ...
        countByCategory.set(normalizedCategory, (countByCategory.get(normalizedCategory) ?? 0) + item.count);
      });
    ...
    const booleanTimeSeries2 =
      isBoolean && isSameScore && mode === "two" ? apiData.timeSeriesCategorical1 : apiData.timeSeriesCategorical2;
    ...
    const distribution1 = isBoolean
      ? buildBooleanDistribution(apiData.timeSeriesCategorical1, booleanCategories)
      : categories
    ...
    const distribution1Matched = isBoolean
      ? buildBooleanDistribution(apiData.timeSeriesCategorical1Matched, booleanCategories)
      : categories
    ...
    const distribution2Matched = isBoolean
      ? buildBooleanDistribution(booleanTimeSeries2Matched, booleanScore2Categories)
      : score2Categories
