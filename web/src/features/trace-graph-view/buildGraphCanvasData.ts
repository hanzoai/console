...
export function transformLanggraphToGeneralized(
  ...
  const transformedData = filteredData.map((obs) => {
    ...
    // Transform system nodes to Hanzo system nodes
    if (obs.node === LANGGRAPH_START_NODE_NAME) {
      transformedObs.name = HANZO_START_NODE_NAME;
      transformedObs.id = HANZO_START_NODE_NAME;
    } else if (obs.node === LANGGRAPH_END_NODE_NAME) {
      transformedObs.name = HANZO_END_NODE_NAME;
      transformedObs.id = HANZO_END_NODE_NAME;
    }
...
export function buildGraphFromStepData(
  ...
  data.forEach((obs) => {
    ...
    if (obs.parentObservationId) {
      ...
      if (!parent && node === null) {
        if (!nodeToObservationsMap.has(HANZO_END_NODE_NAME)) {
          nodeToObservationsMap.set(HANZO_END_NODE_NAME, []);
        }
        nodeToObservationsMap.get(HANZO_END_NODE_NAME)!.push(obs.id);
      }
