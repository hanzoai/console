...
import { type ChartBin, type ChartData, type TimeseriesDataTransformer } from "@/src/features/scores/types";
...
export class DashboardCategoricalScoreAdapter implements TimeseriesDataTransformer {
  ...
  toChartData(): ChartData {
    const { chartData, chartLabels } = transformCategoricalScoresToChartData(this.data, this.timestamp, this.agg);
...
export class CompareViewAdapter implements TimeseriesDataTransformer {
  constructor(
    private runMetrics: Map<string, { chartData: ChartBin[]; chartLabels: string[] }>,
    private key: string,
  ) {}
  ...
  toChartData(): ChartData {
    return {
      ...
      chartLabels: uniqueAndSort(this.runMetrics.get(this.key)?.chartLabels ?? []),
    };
  }
}
