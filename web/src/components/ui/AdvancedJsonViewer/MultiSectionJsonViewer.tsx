import { useMemo, useEffect, useRef, useImperativeHandle, forwardRef, type RefObject } from "react";
import type { JsonSection, PartialJSONTheme, StringWrapMode, SectionContext, ExpansionState } from "./types";
...
import { SimpleMultiSectionViewer, type SimpleMultiSectionViewerHandle } from "./SimpleMultiSectionViewer";
 ...
 */
export const MultiSectionJsonViewer = forwardRef<MultiSectionJsonViewerHandle, MultiSectionJsonViewerProps>(
  function MultiSectionJsonViewer(
    ...
    const viewerRef = useRef<VirtualizedMultiSectionViewerHandle | SimpleMultiSectionViewerHandle>(null);
    ...
    const { tree, expansionVersion, handleToggleExpansion, handleToggleSectionExpansion } = useMultiSectionTreeState({
      sectionConfigs,
      searchQuery,
    ...
    const matchCounts = useMemo(() => {
      return tree && searchMatches.length > 0 ? getMatchCountsPerNode(tree, searchMatches) : undefined;
    }, [tree, searchMatches]);
