...
import { getNodeByIndex, treeNodeToFlatRow, findNodeIndex, findSectionHeaderIndex } from "./utils/treeNavigation";
...
export const VirtualizedMultiSectionViewer = memo(
  forwardRef<VirtualizedMultiSectionViewerHandle, VirtualizedMultiSectionViewerProps>(
          ...
          >
            {virtualItems.map((virtualRow) => {
              ...
              if (node.nodeType === "section-header") {
                const jsonSection = node.sectionKey ? sectionsMap.get(node.sectionKey) : null;
                const sectionContext = {
                  sectionKey: node.sectionKey || "",
                  rowCount: node.totalDescendantCount ?? node.visibleDescendantCount,
                ...
                const title =
                  jsonSection?.title ||
                  (node.sectionKey ? node.sectionKey.charAt(0).toUpperCase() + node.sectionKey.slice(1) : "");

                // Filter media for this section
                const sectionMedia = media?.filter((m) => m.field === node.sectionKey);

                // Get comment count for this section
                const sectionCommentCount = getCommentCountForSection(node.sectionKey, commentedPathsByField);
