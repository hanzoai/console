...
import type { JSONTheme, StringWrapMode, JsonSection, SectionContext } from "./types";
...
import { getCommentRangesForRow, getCommentCountForSection, type CommentedPathsByField } from "./utils/commentRanges";
...
export interface SimpleMultiSectionViewerProps {
  ...
  theme: JSONTheme;
  defaultRenderHeader?: (context: SectionContext & { title: string }) => React.ReactNode;
  searchQuery?: string;
...
export const SimpleMultiSectionViewer = memo(
  forwardRef<SimpleMultiSectionViewerHandle, SimpleMultiSectionViewerProps>(function SimpleMultiSectionViewer(
    {
      tree,
      sections,
      expansionVersion,
    ...
    useImperativeHandle(
      ...
      () => ({
        scrollToSection: (sectionKey: string) => {
          ...
          const sectionElement = container.querySelector(`[data-section-key="${sectionKey}"]`);
          if (sectionElement) {
    ...
    const renderJsonRow = (node: (typeof visibleNodes)[0], index: number) => {
      ...
      const commentRanges = getCommentRangesForRow(row, node.sectionKey, commentedPathsByField);
        ...
        <div style={{ position: "relative" }}>
          {sectionGroups.map((sectionGroup) => {
            ...
            const sectionContext = {
              ...
              rowCount: sectionGroup.header.totalDescendantCount ?? sectionGroup.content.length,
            ...
            const title =
              jsonSection?.title || (sectionKey ? sectionKey.charAt(0).toUpperCase() + sectionKey.slice(1) : "");

            // Filter media for this section
            const sectionMedia = media?.filter((m) => m.field === sectionKey);

            // Get comment count for this section
            const sectionCommentCount = getCommentCountForSection(sectionKey, commentedPathsByField);
            ...
            return (
              <div
                ...
                style={{
                  width: stringWrapMode === "wrap" ? "100%" : "fit-content",
                  backgroundColor: sectionGroup.header.backgroundColor || theme.background,
                }}
              >
                ...
                >
                  {sectionGroup.content.map((node, index) => renderJsonRow(node, index))}
                </div>
