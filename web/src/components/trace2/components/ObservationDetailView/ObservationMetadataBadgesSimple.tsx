...
export function LatencyBadge({ latencySeconds }: { latencySeconds: number | null }) {
  if (latencySeconds == null) return null;

  return <Badge variant="tertiary">Latency: {formatIntervalSeconds(latencySeconds)}</Badge>;
}

export function TimeToFirstTokenBadge({ timeToFirstToken }: { timeToFirstToken: number | null | undefined }) {
  if (timeToFirstToken == null) return null;

  return <Badge variant="tertiary">Time to first token: {formatIntervalSeconds(timeToFirstToken)}</Badge>;
}
...
export function LevelBadge({ level }: { level: string | null | undefined }) {
  ...
  return (
    <Badge variant={level === "ERROR" ? "destructive" : level === "WARNING" ? "warning" : "tertiary"}>{level}</Badge>
  );
}

export function StatusMessageBadge({ statusMessage }: { statusMessage: string | null | undefined }) {
