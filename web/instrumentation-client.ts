...
const isEuOrUsRegionNonHipaa =
  process.env.NEXT_PUBLIC_HANZO_CLOUD_REGION !== undefined
    ? ["EU", "US"].includes(process.env.NEXT_PUBLIC_HANZO_CLOUD_REGION)
    : false;
...
Sentry.init({
  ...
  tracesSampleRate: process.env.NEXT_PUBLIC_HANZO_TRACING_SAMPLE_RATE
    ? Number(process.env.NEXT_PUBLIC_HANZO_TRACING_SAMPLE_RATE)
    : 0,

  // Capture Replay for 100% of all sessions,
  // plus for 100% of sessions with an error
  replaysSessionSampleRate: process.env.NEXT_PUBLIC_HANZO_TRACING_SAMPLE_RATE
    ? Number(process.env.NEXT_PUBLIC_HANZO_TRACING_SAMPLE_RATE)
    : 0,
